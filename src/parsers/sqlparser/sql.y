...
%token <bytes> SCHEMA TABLE DESCRIPTOR INDEX VIEW TO IGNORE IF UNIQUE PRIMARY COLUMN SPATIAL FULLTEXT KEY_BLOCK_SIZE CHECK
...
// Type Tokens Extend
%token <bytes> INT8 INT16 INT32 INT64
%token <bytes> UINT8 UINT16 UINT32 UINT64
%token <bytes> FLOAT32 FLOAT64
%token <bytes> ENUM8 ENUM16
...
%token <bytes> COLLATION DATABASES TABLES VITESS_METADATA VSCHEMA FULL PROCESSLIST COLUMNS FIELDS ENGINES ENGINE PLUGINS
...
| CREATE DATABASE not_exists_opt ID database_option
  {
      $$ = &DBDDL{Action: CreateStr, DBName: string($4), Options:$5}
  }
...
table_spec:
  '(' table_column_list ')' table_option
  {
    $$ = $2
    $$.Options = $4
  }
...
| FLOAT32 decimal_length_opt
  {
    $$ = ColumnType{Type: string($1)}
    $$.Length = $2.Length
    $$.Scale = $2.Scale
  }
| FLOAT64 decimal_length_opt
  {
    $$ = ColumnType{Type: string($1)}
    $$.Length = $2.Length
    $$.Scale = $2.Scale
...
| ENUM8 '(' enum_values ')' charset_opt collate_opt
  {
    $$ = ColumnType{Type: string($1), EnumValues: $3, Charset: $5, Collate: $6}
  }
| ENUM16 '(' enum_values ')' charset_opt collate_opt
  {
    $$ = ColumnType{Type: string($1), EnumValues: $3, Charset: $5, Collate: $6}
  }
...
| ENGINE '=' table_opt_value
  {
    $$ = &TableOption{Engine:$3}
  }
| ENGINE '=' table_opt_value '(' table_opt_value ')'
  {
    $$ = &TableOption{Engine:$3, Formatter:$5}
  }
...
| ID openb table_valued_function_arguments_opt closeb as_opt
  {
...
table_valued_function_argument:
 table_valued_function_argument_value
  {
    $$ = &TableValuedFunctionArgument{Value: $1}
  }
