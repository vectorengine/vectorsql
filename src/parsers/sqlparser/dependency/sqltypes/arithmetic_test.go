...
func TestDivide(t *testing.T) {
	tcases := []struct {
		...
		v2:  NewFloat64(0.5),
		err: errors.New("BIGINT is out of range in 1.7976931348623157e+308 / 0.5"),
	}}

	for _, tcase := range tcases {
		got, err := Divide(tcase.v1, tcase.v2)

		if !errorEquals(err, tcase.err) {
			t.Errorf("%v %v %v", printValue(tcase.v1), printValue(tcase.v2), (err))
			t.Errorf("Divide(%v, %v) error: %v, want %v", printValue(tcase.v1), printValue(tcase.v2), (err), (tcase.err))
		}
...
func TestMultiply(t *testing.T) {
	tcases := []struct {
		...
		v2:  NewInt64(2),
		err: errors.New("BIGINT value is out of range in 9223372036854775807 * 2"),
	}, {
		// testing for underflow of uint64*max.uint64
		v1:  NewInt64(2),
		v2:  NewUint64(math.MaxUint64),
		err: errors.New("BIGINT UNSIGNED value is out of range in 18446744073709551615 * 2"),
	}, {
		v1:  NewUint64(math.MaxUint64),
		v2:  NewUint64(1),
		...
		v2:  NewInt64(3),
		err: errors.New("BIGINT UNSIGNED value is out of range in 9223372036854775807 * 3"),
	}}
	...
	for _, tcase := range tcases {
		...
		if !errorEquals(err, tcase.err) {
			t.Errorf("Multiply(%v, %v) error: %v, want %v", printValue(tcase.v1), printValue(tcase.v2), (err), (tcase.err))
		}
		if tcase.err != nil {
			continue
		}
...
func TestSubtract(t *testing.T) {
	tcases := []struct {
		...
		v1:  NewInt64(math.MinInt64),
		v2:  NewInt64(1),
		err: errors.New("BIGINT value is out of range in -9223372036854775808 - 1"),
	}, {
		v1:  NewUint64(4),
		v2:  NewInt64(5),
		err: errors.New("BIGINT UNSIGNED value is out of range in 4 - 5"),
	}, {
		// testing uint - int
		v1:  NewUint64(7),
		v2:  NewInt64(5),
		...
		v1:  NewInt64(math.MinInt64),
		v2:  NewUint64(0),
		err: errors.New("BIGINT UNSIGNED value is out of range in -9223372036854775808 - 0"),
	}, {
		...
		v1:  NewInt64(-1),
		v2:  NewUint64(2),
		err: errors.New("BIGINT UNSIGNED value is out of range in -1 - 2"),
	}, {
		v1:  NewInt64(2),
		v2:  NewUint64(1),
		out: NewUint64(1),
		...
		v1:  NewUint64(2),
		v2:  NewUint64(4),
		err: errors.New("BIGINT UNSIGNED value is out of range in 2 - 4"),
	}, {
		// testing uint - (- int)
		v1:  NewUint64(1),
		v2:  NewInt64(-2),
	...
	for _, tcase := range tcases {
		...
		if !errorEquals(err, tcase.err) {
			t.Errorf("Subtract(%v, %v) error: %v, want %v", printValue(tcase.v1), printValue(tcase.v2), (err), (tcase.err))
		}
		if tcase.err != nil {
			continue
		}
...
func TestAdd(t *testing.T) {
	tcases := []struct {
		...
		v1:  NewInt64(math.MinInt64),
		v2:  NewInt64(-2),
		err: errors.New("BIGINT value is out of range in -9223372036854775808 + -2"),
	}, {
...
func TestCast(t *testing.T) {
	...
	for _, tcase := range tcases {
		...
		if !errorEquals(err, tcase.err) {
			t.Errorf("Cast(%v) error: %v, want %v", tcase.v, (err), (tcase.err))
		}
		if tcase.err != nil {
			continue
		}
...
func TestToFloat64(t *testing.T) {
	...
	for _, tcase := range tcases {
		...
		if !errorEquals(err, tcase.err) {
			t.Errorf("ToFloat64(%v) error: %v, want %v", tcase.v, (err), (tcase.err))
		}
		if tcase.err != nil {
			continue
		}
...
func TestNewIntegralNumeric(t *testing.T) {
	...
	for _, tcase := range tcases {
		...
		if err != nil && !errorEquals(err, tcase.err) {
...
func TestCastFromNumeric(t *testing.T) {
	tcases := []struct {
		...
		typ: VarBinary,
		v:   numeric{typ: Int64, ival: 1},
		err: errors.New("unexpected type conversion to non-numeric: VARBINARY"),
	}}
	for _, tcase := range tcases {
