...
func (ts *timeseries) Record(n time.Time) {
	ts.Add(float64(time.Since(n)) / float64(time.Second))
}
...
func (mm multimetric) Record(n time.Time) {
	mm.Add(float64(time.Since(n)) / float64(time.Second))
}
...
func (c *counter) Record(n time.Time) {
	c.Add(float64(time.Since(n)) / float64(time.Second))
}
...
func (g *gauge) Record(n time.Time) {
}
...
func (h *histogram) Record(n time.Time) {
	h.Add(float64(time.Since(n)) / float64(time.Second))
}
